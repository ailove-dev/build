...
if [ -d "$GIT_REPOSITORIES_PATH/$PROJECT" ]; then
    $SUDO_PATH -u $GIT_USERNAME ssh -o StrictHostKeyChecking=no $PROJECT.$PRO_HOSTNAME "cd $WWW_PATH/$PROJECT/repo/master; GIT_SSL_NO_VERIFY=true git pull; TAG=$((`GIT_SSL_NO_VERIFY=true git tag -l | wc -l`+1)); GIT_SSL_NO_VERIFY=true git tag $TAG; GIT_SSL_NO_VERIFY=true git push origin --tags; /bin/sh /srv/admin/bin/update-revision.sh $PROJECT master"
else
