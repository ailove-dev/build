...
if [ -d "$GIT_REPOSITORIES_PATH/$PROJECT" ]; then
    $SUDO_PATH -u $GIT_USERNAME ssh -o StrictHostKeyChecking=no $PROJECT.$TEST_HOSTNAME "cd $WWW_PATH/$PROJECT/repo/test; GIT_SSL_NO_VERIFY=true git pull; /bin/sh /srv/admin/bin/update-revision.sh $PROJECT test"
else
